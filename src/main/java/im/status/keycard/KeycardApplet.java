...
public class KeycardApplet extends Applet {
   ...
   * This returns a public key corresponding to the private key generated by:
   *    sha256(salt, idPrivate)
   * @param apdu - the JCRE-owned APDU object
   * * P1 and P2 are not checked
   * * Data contains a uint32 representing the timestamp. This can be empty, as it is not validated here.
   * 
   * @returns [TLV_PHONON_NEW_SALT][TLV_SHORT][2][saltSlotIndex][TLV_PUB_KEUY][65][pubKey]
   */
  private void phononNewSalt(APDU apdu) {
    ...
    apduBuffer[off] = (byte) Crypto.KEY_PUB_SIZE; off++; // Length of int
    byte[] priv = getPhononKey(salt);
    byte[] pub = privBytesToPubBytes(priv);
    Util.arrayCopy(pub, (short) 0, apduBuffer, (short) off, (short) Crypto.KEY_PUB_SIZE); 
  ...
  private void phononGetReceivePubKey(APDU apdu) {
    ...
    // Ensure there is a salt present
    if (phonon.saltIsEmpty(i) == true) {
      ISOException.throwIt(ISO7816.SW_RECORD_NOT_FOUND);
    }
    byte[] salt = phonon.getSalt(i);
    apduBuffer[0] = TLV_PUB_KEY;
    apduBuffer[1] = Crypto.KEY_PUB_SIZE;
    byte[] priv = getPhononKey(salt);
    byte[] pub = privBytesToPubBytes(priv);
  ...
  // Get a phonon private key based on some salt (this may be a nonce)
  private byte[] getPhononKey(byte[] salt) {
    // private byte[] getPhononKey(byte a, byte b) {
    short saltLen = (short) salt.length;
  ...
  private static byte[] privBytesToPubBytes(byte[] privBuf) {
    ECPrivateKey priv;
    Crypto crypto = new Crypto();
    SECP256k1 secp256k1 = new SECP256k1(crypto);
    priv = (ECPrivateKey) KeyBuilder.buildKey(KeyBuilder.TYPE_EC_FP_PRIVATE, SECP256k1.SECP256K1_KEY_SIZE, false);
    secp256k1.setCurveParameters(priv);
    priv.setS(privBuf, (short) 0, Crypto.KEY_SECRET_SIZE);
    byte[] pubBuf = new byte[Crypto.KEY_PUB_SIZE];
    secp256k1.derivePublicKey(priv, pubBuf, (short) 0);
    return pubBuf;
  }
